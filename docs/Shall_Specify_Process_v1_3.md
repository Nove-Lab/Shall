...
> v1.4 change: the second stage of the dual approval may be deferred to the end of a run, on the user's explicit request (`--auto`). The first stage is unchanged and every phase still closes on it; what moves is the web approval, from once per phase to once per run.
...
* **Dual-approval rule**: every phase closes with two-stage approval — (1) explain the full set of outputs in the terminal and obtain approval, (2) upon approval, immediately sync the phase-complete set to the Shall DB, and the user reviews it as a graph in the Shall web interface for final approval.
  ...
  * **Deferred second stage (`--auto`, v1.4)**: on the user's explicit request the web approval is gathered into one, taken after the last phase over everything the run wrote. Every phase still closes on its terminal approval, and the order of the phases is unchanged — it is fixed by anchoring, not by approval. What it buys is five fewer waits; what it costs is that a rejection lands after everything below it was built, and the repair is the revision path from that phase down. The first stage is not negotiable and the web approval is not skipped — only moved. (Precedent: the same rule already yields where the rite would buy nothing, in Phase 6's empty-set close.)

...
| `/shall:plan <direction>` over an **unapproved** specification | walk the chain, name the ids, send the person to `/shall:specify`, write nothing | named all five ids with their statements, explained that a plan on unapproved nodes is a plan whose tasks can never start, sent them to `/shall:specify`, wrote nothing |

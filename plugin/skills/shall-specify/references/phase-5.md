...
Every responsibility from Phase 4 green in `shall status --json`: a requirement is anchored by a line written into a responsibility's file, so a responsibility still yellow is a file you would be rewriting under a person's open judgement. Phase 3's dictionary is still the controlled vocabulary; a concept it lacks sends you to [phase 3](./phase-3.md) step 1 before the sentence is written.

## What a requirement is

**One SHALL or MUST sentence carrying exactly one behaviour.** Four rules, all of them yours to keep:

- **The word is there** — `SHALL` or `MUST` appears in the statement. **Nothing verifies this**: not `shall check`, not the loader, not any door. The process document called it machine-verified; in Shall it is a rule of authorship, kept by you and caught by the person reviewing the node.
...
5. **Give every approved requirement at least one unit criterion.** Phase 2 put integrative criteria on scenarios under the same relation name from a different parent; these are the unit verdicts, and they are their own nodes.
   ...
   - **It says what is checked and how it is checked**, in enough detail that somebody who was not in this conversation can run the judgement and reach the same verdict. The starting file says where that goes.
   - Where the judgement is quantitative, it says what is measured and what value counts as met, without naming a technology to measure it with.
...
|---|---|---|---|
| a responsibility reads two ways | which behaviour must this sentence carry? | 2–4 readings | `Reading` |
...
| a quantitative judgement has no number | what value counts as met? | 2–4 targets | `Target` |
...
| Every requirement is one SHALL/MUST sentence carrying one behaviour | you read them — nothing verifies the SHALL |
...
| Every criterion says what is checked and how, gives a measured value and its target wherever the judgement is quantitative, and carries no test-case detail | you read them — `shall status` reports no body |
...
| a requirement carrying two behaviours | step 1 — split, and anchor both |

...
5. **Resolve what is left by asking.** None of the four types this phase writes may assume — `ASSUMES` runs from a goal, a responsibility or a requirement, and none of those is being written here — so a default has nowhere to hang and the point is settled in the conversation instead. If the default genuinely belongs to a goal, that is a Phase 1 revision: take it there rather than smuggling it in.

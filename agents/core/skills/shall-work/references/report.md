...
2. **`git log <moment>..HEAD`, with the files each commit touched.** Read the commits and the changes against the specification and work out which work item or requirement family each belongs to. A mapping you are not sure of is a question, never a guess: a log filed against the wrong work item is a claim about work nobody did.

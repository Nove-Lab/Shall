...
- **A work item targets none, one or several criteria.** A structural item targets none and is done when a person closes the completion report claiming it; a functional item targets the criteria its definition of done makes judgeable there and then — the criterion's own evaluation process runs whole on what the item and what it waits on supply. An aim is a promise of a verdict: evidence is filed only under an item that aims, and once every item aiming at a criterion is done, nothing files any. A criterion the item serves and cannot judge is said in its Scope, not aimed at. A definition of done is never the criterion's sentence again.
...
|---|---|
| `shall status [--scope <path>]… [--json]` | every node with its color and the reason, the problem sentence a rule wrote against it, a standing rejection's rationale in full, a criterion's open/closed mark and its `aims` — `pending`, `spent` when every work item aiming at it is done and it is still open (with the sentence naming them, `spentAim`), `none` when nothing aims at it — a work item's blocked/ready/in_review/done (`workItemState`), a deletion proposal, and the relations written in its file |

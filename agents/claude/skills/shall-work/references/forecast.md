...
2. **Pick the bundle** exactly as the spine would have, and say which items, why, and the one line of approach you would have given for each. Nothing is confirmed and nobody is asked: this is the pick the cycle would have put at stop 1.
...
- **Nothing the specification does not say.** Where the spec is silent about what a work item would produce, the forecast is silent too. An invented deliverable reads exactly like a real one in a transcript somebody skims.

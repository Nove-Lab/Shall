...
- **The work stretch plans against the code before it writes any.** A work item is a promise cut from the specification: its scope and its definition of done say what must be true when it is done, and say nothing about which files change or what the code already there will allow. So `/shall:work` now makes a plan against this repository — at the level of files and functions — before the first file is opened for the code, and the plan is said rather than thought: it is text in front of the person, and nothing is written under the item until it is there. Going from the look back straight into the code is the mistake the rule exists to stop; the look back read the specification's past, and the plan reads the code.
- **The project's own procedure comes first.** If a project already says how work is planned before it is built — a procedure its rules file names, or a skill that exists for it — that is what is used, standing above the agent's own tool the way the project's conventions stand above the skill. Only when a project names none does the agent reach for its own: Claude enters plan mode and leaves it on the user's approval, which is the one stop the stretch adds; Codex writes the same plan in its message and ends the turn on it as a question. Under `--auto` neither stops — the plan is written into the journal beside the approach line, so the record still says how the item was going to be done before it says how it was.
- The self-check gains a line for it: the plan was made before the code, and you can name where.

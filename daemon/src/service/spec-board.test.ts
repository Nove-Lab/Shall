...
/**
 * The Work Board over real folders and real books.
 *
 * WHAT IS ASSERTED IS THE GATE AND THE WORDS. Which work items appear, what makes
...
/** Goal down to a work item, with a journal and one log addressing the work item. */
...
/** The whole graph, every node approved: one work item, ready to work on. */
async function greenProject(): Promise<RegistryProject> {
  ...
  // blocked, and a log under a blocked work item is red (`premature`) — the door
...
describe("the Implement half", () => {
  ...
  test("hides it while one node above it is refused, and offers it again once fixed", async () => {
    ...
    // chain back together and the work item returns of its own accord.
  ...
  test("waits on a prerequisite and is offered the moment it closes", async () => {
    ...
    await edge(project, "TARGETS", "WI-0002", "AC-0001");
    await edge(project, "DEPENDS_ON", "WI-0001", "WI-0002");
    await node(project, "WorkLog", "WL-0002");
    await edge(project, "LOGS", "J-0001", "WL-0002");
    await edge(project, "ADDRESSES", "WL-0002", "WI-0002");
    await node(project, "CompletionReport", "CR-0002");
    await edge(project, "SUBMITS", "WL-0002", "CR-0002");
    await edge(project, "CLAIMS", "CR-0002", "WI-0002");
    // Wiring the new work item moved the module's own file (ALLOCATES is written
    // there) and the work item's (DEPENDS_ON is written in its own), so everything
    ...
    // their work items are blocked, and a log under a blocked work item is red.
    await approveSpecNodes({
      projectId: project.id,
      ids: [
    ...
    // WL-0001 is left alone on purpose: adding the dependency put WI-0001
    // behind WI-0002, so the work already logged under it is `premature` red —
  ...
  test("offers neither work item when the two of them wait on each other", async () => {
    // The Implement half's whole promise is that everything on it can be
    // started. Two work items waiting on each other can never satisfy that, so both
  ...
  test("is exactly the work items the review badges as ready", async () => {
    const project = await greenProject();
    const ready = (await reviewSpec(project.id)).statuses
      .filter((status) => status.workItemState === "ready")
...
describe("the Fix Spec half", () => {
  test("carries a rejection's rationale whole, and puts it first", async () => {
    ...
    // chain over WI-0001 is no longer green, the work item is blocked, and the work
  ...
  test("says a file that would not read once, with its id", async () => {
    ...
    const rows = (await workBoard(project.id)).fixSpec.filter(
  ...
  test("names every referrer of a hole in one row", async () => {
    ...
    const row = (await workBoard(project.id)).fixSpec.find(
      (held) => held.id === "R-0404",
    );
    assert.ok(row !== undefined);
    assert.equal(
...
describe("a book that will not read", () => {
  test("refuses the whole board and says which book", async () => {
    ...
    const refusal = await refused(workBoard(project.id));

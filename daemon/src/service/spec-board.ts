import { workBoardOf, type WorkBoard } from "@shall/core/arith";
...
/**
 * The Work Board: what the specification needs fixed, and what is ready to be
 * worked on.
 *
 ...
 * `workBoardOf` decides what is red, what is startable and in what order, and
 ...
 * `workBoard` below has an id to look up, `boardAt` a path to walk up from —
 ...
 * approval makes a chain green, an acceptance makes a work item done — so a board
 * served over an unreadable book would show work as ready that a person has
 * already refused.
 */
export async function boardOver(paths: SpecPaths): Promise<WorkBoard> {
  const ledgers = await requireLedgers(paths, "board");
  return workBoardOf(await loadGraph(paths.specDir), ledgers);
}
 ...
 * them carries.
 */
export async function workBoard(projectId: string): Promise<WorkBoard> {

...
- **frontmatter는 기계의 것, 본문은 저자의 것.** 펜스 위에는 그래프가 필요로 하는
  ...
  작업이 만든 커밋의 sha 목록 — 메시지는 git의 것), 그리고 기계가 스스로 쓰는 블록 둘 —
...
4. **커밋한다.** 이력·병합·리뷰는 git이 한다. 데몬은 스스로 커밋하지 않는다 —
   사람이 터미널에서 하거나, 웹의 **Commit Spec** 버튼(git 프로젝트에서 spec에
   미커밋 변경이 있을 때 활성)으로 한다. 승인본 diff와 삭제 복원은 git에 커밋된
   만큼만 가능하다.

노드를 **없애는** 길도 파일 계약이다: 에이전트는 파일을 지우지 않고, frontmatter에
`deletionProposed:` 블록(`by`·`rationale` 두 키)을 적는다. 그 블록은 서명된
...
- 쓰기는 spec 폴더별 **직렬 큐**를 지나고, `<이름>.<pid>.tmp` + rename으로 앉는다.
  읽기는 큐를 타지 않는다. 승인 문은 읽기·서명·쓰기를 **한 큐 턴**에 한다 — 큐
  건너편에서 계산한 해시는 그 사이에 착지한 저장으로 태어나자마자 낡을 수 있다
...
- reviewGraph — 상태 목록, 답하는 파일이 없는 id와 그것을 이름하는 참조들, 읽히지
  않는 파일들을 한 번에 조립한다. `spec.review`와 `shall check`의 gap이 같은
  산술을 읽는다
...
- **동결된 형식** — frontmatter 블록(`short_name`·`name`·`edges`, 그리고 기계
  블록 `deletionProposed`·`approval` — `approval`은 언제나 마지막) + 자유 마크다운
  본문. UTF-8, BOM 없음, LF, 말미 개행 하나. 키 순서·엣지 순서가 각각 하나뿐이고,
  본문은 저자의 바이트 그대로다
- **승인 페이로드** — `<type>/<id>` 한 줄 + approval 블록만 뺀 정칙 파일. 경로
  정체성이 앞에 붙어 승인된 파일을 다른 id로 복사해도 서명이 따라가지 않고,
  `approval`이 파일 맨 끝이라 사람이 손으로도 검증할 수 있다. 해시가 정칙 emit
  기준이므로 되읽어 같은 노드가 되는 재포맷은 승인을 살려 둔다
...
- 스펙 프로시저 — 노드·엣지 읽기와 쓰기 다섯, 그리고 리뷰 표면 일곱(`review`·
  ...
  에이전트의 계약은 파일 전용이라 이 문을 두드릴 이유가 없고, 키의 집은 deny 규칙이
...
- **훅을 설치하지 않고, 스스로 커밋하지 않는다.** 스펙의 이력은 사람이 만든다 —
  터미널에서, 또는 웹의 Commit Spec 버튼으로(그때도 커밋은 spec 폴더 범위의 부분
  커밋 하나다). 브랜치 표시는 여전히 `.git/HEAD`를 직접 읽어 바이너리를 띄우지
  않지만, **상태를 다루는 일곱**(status·add·commit·log·show·rev-list·init)은
  `git-cli` 한 문 뒤에서 바이너리를 띄운다 — 승인본 재구성, 삭제 복원, 커밋 버튼,
  프로젝트 생성 시 `git init`이 그 소비자다
- `~/.shall/key` — 머신당 하나, 최초 기동에 발급, 0600. 승인 태그의 HMAC 키라
  repo 밖에 산다. **키가 머신 단위라는 것은 clone한 기계에서 green이 전부
  forged-yellow로 읽힌다는 뜻이고, 그건 버그가 아니라 설계다** — 그 기계의 사람이
  다시 읽고 다시 서명한다. 키를 잃으면 전 노드 재승인이고, 명세가 정한 비용이다
...
- Spec plane — 그래프 캔버스(React Flow)와 노드 상세, 그리고 리뷰 표면. 카드마다
  ...
  승인/반려 두 버튼. missing·broken은 툴바의 problems 다이얼로그에서 복원한다.
  저장은 그 노드의 파일을 정칙 바이트로 다시 쓴다
...
- `shall check` — `파일 — 문장`을 출력하고 problem이나 gap이 있으면 exit 1.
  승인 여부(yellow·green)는 말하지 않는다 — 그건 키를 쥔 리뷰의 것이다.
...
- **승인 페이로드** — `<type>/<id>` 한 줄 + approval 제외 정칙 파일, sha256과
  HMAC-SHA256. 이 정의가 바뀌면 서명된 모든 파일이 yellow로 돌아간다
